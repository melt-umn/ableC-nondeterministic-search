...
int main() {
  int result1 = -1;
  bool success1 = invoke(search_sequential_dfs, &result1, baz(1, 2, 2, 3));
  printf("baz(1, 2, 2, 3) %s: %d\n", success1? "succeeded" : "failed", result1);
  
  int result2 = -1;
  bool success2 = invoke(search_sequential_dfs, &result2, baz(1, 2, 3, 2));
  printf("baz(1, 2, 3, 2) %s: %d\n", success2? "succeeded" : "failed", result2);
  
  int result3 = -1;
  bool success3 = invoke(search_sequential(1), &result3, baz(2, 1, 2, 3));
  printf("baz(2, 1, 2, 3) %s: %d\n", success3? "succeeded" : "failed", result3);
  
  int result4 = -1;
  bool success4 = invoke(search_sequential(2), &result4, baz(2, 1, 3, 2));
